...
function titleA4(){
    ...
    $conn = connection();
    //select code periode 
    $sql_periode="SELECT code_periode FROM periode where typePeriode='".$GLOBALS['periode']."' and numPeriode='".$GLOBALS['trimestre']."' and anneePeriode='".$GLOBALS['annee']." ';";
    ...
    $output.='</h3><br><br>
            ...
            <h5 style="text-align:left;">Coût du repas: '.$coutRepas_aff.'</h5>';                    
    return $output;
} 
function sections(){
    $output = '';      
...
function designA4(){
    ...
    $row10 = mysqli_fetch_assoc($result10);
            ...
            $row10 = mysqli_fetch_assoc($result10);
               $output .= '<tr style="text-align:center;font-size:12px;"> <td> ' . $row1["nomDesignA4"]. '</td>';
                while ($row3 = mysqli_fetch_assoc($result3)){
                    ...
                    $result0 = mysqli_query($conn, $sql0);
                    while($row0 = mysqli_fetch_assoc($result0)) {
                        $sql2 = "SELECT nbrerepas, mtA4 FROM designA4_sce_periode_data where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignA4=$row1[code_designA4]";
            ...
            $sql6= "SELECT sum(nbrerepas) as somnbrerepas ,sum(mtA4) as somMt FROM designA4_sce_periode_data where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeDesignA4= $row1[code_designA4]";
    ...
    $row10 = mysqli_fetch_assoc($result10);
            while($row1 = mysqli_fetch_assoc($result1)) {
               ...
               $n='1';               
                ...
                where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and codeDesignA4= $row1[code_designA4]";
                $result6 = mysqli_query($conn, $sql6);
                $row6 = mysqli_fetch_assoc($result6);
                $output .= '<td>' .$row6["somnbrerepas"]. '</td>
                            <td>' .$row6["somMt"]. '</td>';
...
function totalSce_pdf_p2(){
    ...
    while ($row3 = mysqli_fetch_assoc($result3)){
        ...
        //Total
        $sql8 = "SELECT sum(nbrerepas) as somnbrerepas ,sum(mtA4) as somMt FROM designA4_sce_periode_data where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']."";
        $result8 = mysqli_query($conn, $sql8);
        $row8 = mysqli_fetch_assoc($result8);
        $output .= '<td>' .$row8["somnbrerepas"]. '</td>
                <td>' .$row8["somMt"]. '</td>';
...
if(isset($_POST["generate_pdf"]))  
 {  
      ...
      $obj_pdf->Output('A4_Trimestriel_'.$GLOBALS['trimestre'].'_'.$GLOBALS['annee'].'.pdf', 'I'); 
    }  
 ?>  
 <!DOCTYPE html>  
 <html>  
    ...
    <body>  
                ...
                <?php $conn= connection();
                    //Title
                        $title=title();
                        echo"<h4>" .$title."</h4>";$titleA4= titleA4();
                        echo "".$titleA4."";
                        ?>
                <br>
