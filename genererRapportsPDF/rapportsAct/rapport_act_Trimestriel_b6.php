    ...
    $result1 = mysqli_query($conn, $sql1);    
            while($row1 = mysqli_fetch_assoc($result1)) {
               ...
               $output .= '<td> ' . $row_code["codeB6"]. '</td>';
                    ...
                    $result0 = mysqli_query($conn, $sql0);
                    while($row0 = mysqli_fetch_assoc($result0)) {
                        $sql2 = "SELECT nbreB6 FROM designB6_sce_periode_data where  periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB6=$row1[code_designB6]";
                        $result2 = mysqli_query($conn, $sql2);
                        $row2 = mysqli_fetch_assoc($result2);  
                        $output .= '<td>' . $row2["nbreB6"]. '</td>
                                    <td>' . $row2["nbreB6"]*$row_code['codeB6']. '</td>';
                    }
            $sql6= "SELECT sum(nbreB6) as somnbre FROM designB6_sce_periode_data where  periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeDesignB6= $row1[code_designB6]";
...
function totalSce(){
    ...
    $output .= '<tr  style="background-color: #BEBEBE;text-align:center;font-size:12px;"> <td  >TOTAL</td>';
        ...
        while(($row0 = mysqli_fetch_assoc($result0))) {
            ...
            $result1 = mysqli_query($conn, $sql1);
            while($row1 = mysqli_fetch_assoc($result1)) {
                $sql_sumNbr = "SELECT nbreB6,codeB6 FROM designB6_sce_periode_data where  periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB6=$row1[code_designB6]";
...
if(isset($_POST["generate_pdf"]))  {  
     ...
     $content1.= totalSce_pdf();
      ...
      $obj_pdf->Output('B6_Trimestriel_'.$GLOBALS['trimestre'].'_'.$GLOBALS['annee'].'.pdf', 'I'); 
 }  
 ?>  
 <!DOCTYPE html>  
 <html>  
