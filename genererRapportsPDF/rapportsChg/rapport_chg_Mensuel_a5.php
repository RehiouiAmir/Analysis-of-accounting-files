            ...
            $row10 = mysqli_fetch_assoc($result10);
               $output .= '<tr style="text-align:center;font-size:12px;"> <td> ' . $row1["nomDesignA5"]. '</td>';
                while ($row3 = mysqli_fetch_assoc($result3)){
                    ...
                    $result0 = mysqli_query($conn, $sql0);
                    while($row0 = mysqli_fetch_assoc($result0)) {
                        $sql2 = "SELECT mtA5 FROM designA5_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignA5=$row1[code_designA5]";
                        $result2 = mysqli_query($conn, $sql2);
                        $row2 = mysqli_fetch_assoc($result2);  
                        $output .= '<td>' . $row2["mtA5"]. '</td>';
                    }
            }
            $sql6= "SELECT sum(mtA5) as somMt FROM designA5_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeDesignA5= $row1[code_designA5]";
...
function titleA5_pdf(){
    ...
    $output='</h3><br>                  
            <h3 style="text-align:center;text-transform: uppercase;">DEPENSE DE PHARMACIE</h3>';
    return $output;
} 
//PDF-PAGE1
...
function designA5_pdf_p2(){
    ...
    $row10 = mysqli_fetch_assoc($result10);
            ...
            while($row0 = mysqli_fetch_assoc($result0)) {
                ...
                where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and codeDesignA5= $row1[code_designA5]";
                $result6 = mysqli_query($conn, $sql6);
                $row6 = mysqli_fetch_assoc($result6);
                $output .= '<td>' .$row6["somMt"]. '</td>';
                $output .= '</tr>';
...
function totalSce_pdf_p2(){
    ...
    while ($row3 = mysqli_fetch_assoc($result3)){
        ...
        //Total
        $sql8 = "SELECT sum(mtA5) as somMt FROM designA5_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']."";
        $result8 = mysqli_query($conn, $sql8);
        $row8 = mysqli_fetch_assoc($result8);
        $output .= '<td>'.$row8["somMt"]. '</td>';
        $output .= '</tr>';
...
if(isset($_POST["generate_pdf"]))  
 {  
      ...
      $obj_pdf->Output('A5_Mensuel_'.$GLOBALS['mois'].'_'.$GLOBALS['annee'].'.pdf', 'I'); 
    }  
 ?>  
 <!DOCTYPE html>  
 <html>  
