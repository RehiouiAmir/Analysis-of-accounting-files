...
function designB5(){
    $output = '';      
    $conn = connection();
    $sql1 = "SELECT code_designB5, nomDesignB5 FROM designB5 order by nomDesignB5 DESC";
    $result1 = mysqli_query($conn, $sql1);    
            while($row1 = mysqli_fetch_assoc($result1)) {
               $output .= '<tr style="text-align:center;font-size:12px;"> <td> ' . $row1["nomDesignB5"]. '</td>';
               $sql_code="SELECT codeB5 FROM designB5 where code_designB5=$row1[code_designB5]";               
               $result_code = mysqli_query($conn, $sql_code);
               $row_code = mysqli_fetch_assoc($result_code);                 
               $output .= '<td> ' . $row_code["codeB5"]. '</td>';
                    $sql0 = "SELECT code_sce,nomSce FROM service where codeSection='2' order by ordre";
                    $result0 = mysqli_query($conn, $sql0);
                    while($row0 = mysqli_fetch_assoc($result0)) {
                        $sql2 = "SELECT nbreB5 FROM designB5_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB5=$row1[code_designB5]";
                        $result2 = mysqli_query($conn, $sql2);
                        $row2 = mysqli_fetch_assoc($result2);  
                        $output .= '<td>' . $row2["nbreB5"]. '</td>
                                    <td>' . $row2["nbreB5"]*$row_code['codeB5']. '</td>';
                    }
            $sql6= "SELECT sum(nbreB5) as somnbre FROM designB5_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeDesignB5= $row1[code_designB5]";
            $result6 = mysqli_query($conn, $sql6);
            $row6 = mysqli_fetch_assoc($result6);
            $output .= '<td>'.$row6["somnbre"]. '</td>
                        <td>'.$row6["somnbre"]*$row_code['codeB5']. '</td>';
            $output .= '</tr>';
...
function totalSce(){
    ...
    $output .= '<tr  style="background-color: #BEBEBE;text-align:center;font-size:12px;"> <td  >TOTAL</td>';
        ...
        $result0 = mysqli_query($conn, $sql0);
        while(($row0 = mysqli_fetch_assoc($result0))) {
            $sql4 = "SELECT sum(nbreB5) as somnbre FROM designB5_sce_periode_data 
            ...
            $result1 = mysqli_query($conn, $sql1);
            while($row1 = mysqli_fetch_assoc($result1)) {
                $sql_sumNbr = "SELECT nbreB5,codeB5 FROM designB5_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB5=$row1[code_designB5]";
                $result_sumNbr = mysqli_query($conn, $sql_sumNbr);
                $row_sumNbr = mysqli_fetch_assoc($result_sumNbr);
                $sumNbr=$sumNbr+($row_sumNbr['nbreB5']*$row_sumNbr['codeB5']);
            }                
        ...
        //Total
        $sql8 = "SELECT sum(nbreB5) as somnbre FROM designB5_sce_periode_data 
    ...
    $i = 0;
        ...
        while(($row0 = mysqli_fetch_assoc($result0))) {
            if ($GLOBALS['mois']=="Mars"){
                ...
                $result1 = mysqli_query($conn, $sql1);
                while($row1 = mysqli_fetch_assoc($result1)) {
                    $sql_sumNbr = "SELECT nbreB5,codeB5 FROM designB5_sce_periode_data where periode='".$GLOBALS['periode']."' 
                            and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB5=$row1[code_designB5]
                            and ( mois='Mars' or mois='Fevrier' or mois='Janvier');";
                    $result_sumNbr = mysqli_query($conn, $sql_sumNbr);
                    $row_sumNbr = mysqli_fetch_assoc($result_sumNbr);
                    $sumNbr=$sumNbr+($row_sumNbr['nbreB5']*$row_sumNbr['codeB5']);
...
//pdf 
function titleB5_pdf(){

    $output='</h3><br><br><br>                  
            <h3 style="text-align:center;text-transform: uppercase;">ACTIVITE DU LABORATOIRE</h3>
            <h3 style="text-align:right;text-transform: uppercase;">Tableau B5</h3>';
    return $output;
} 
...
function designB5_pdf(){
   $output = '';      
   $conn = connection();
   $sql1 = "SELECT code_designB5, nomDesignB5 FROM designB5 order by nomDesignB5 DESC";
   $result1 = mysqli_query($conn, $sql1);    
           while($row1 = mysqli_fetch_assoc($result1)) {
              $output .= '<tr style="text-align:center;font-size:8px;"> <td> ' . $row1["nomDesignB5"]. '</td>';
              $sql_code="SELECT codeB5 FROM designB5 where code_designB5=$row1[code_designB5]";               
              $result_code = mysqli_query($conn, $sql_code);
              $row_code = mysqli_fetch_assoc($result_code);                 
              $output .= '<td> ' . $row_code["codeB5"]. '</td>';
...
if(isset($_POST["generate_pdf"]))  {  
     ...
     if ($GLOBALS['mois']=="Mars" or $GLOBALS['mois']=="Juin" or $GLOBALS['mois']=="Septembre" or $GLOBALS['mois']=="Decembre"){
      ...
      $obj_pdf->Output('B5_Mensuel_'.$GLOBALS['mois'].'_'.$GLOBALS['annee'].'.pdf', 'I'); 
 }  
 ?>  
 <!DOCTYPE html>  
 <html>  
    ...
    <body>  
                ...
                <br>
                    ...
                    <h5 style="margin-left:78%;">TABLEAU B5 </h5>
                <table>
                ...
                <?php
                    ...
                    //Affichage nbreEX && nbr B  row 
                        ...
                        echo "".$nbre_nbrerepas."";
                    //Affichage des designB5- val_nbrerepas&Montant
                        $designB5= designB5();
                        echo "".$designB5."";
