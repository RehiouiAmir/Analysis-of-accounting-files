...
function designA8(){
    ...
    $sql3 = "SELECT code_section,nomSection FROM section  order by ordreSection";    
    $result3 = mysqli_query($conn, $sql3);
    $sql1 = "SELECT code_designA8, nomDesignA8 FROM designA8 order by nomDesignA8  DESC";
    $result1 = mysqli_query($conn, $sql1);    
    $result10= mysqli_query($conn, $sql1);
    $row10 = mysqli_fetch_assoc($result10);
            while($row1 = mysqli_fetch_assoc($result1)) {
            $row10 = mysqli_fetch_assoc($result10);
               $output .= '<tr style="text-align:center;font-size:12px;"> <td> ' . $row1["nomDesignA8"]. '</td>';
                while ($row3 = mysqli_fetch_assoc($result3)){
                    $sql0 = "SELECT code_sce,nomSce FROM service where codeSection= $row3[code_section] order by ordre";
                    $result0 = mysqli_query($conn, $sql0);
                    while($row0 = mysqli_fetch_assoc($result0)) {
                        $sql2 = "SELECT  mtA8 FROM designA8_sce_periode_data where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignA8=$row1[code_designA8]";
                        $result2 = mysqli_query($conn, $sql2);
                        $row2 = mysqli_fetch_assoc($result2);  
                        $output .= ' <td>'. $row2["mtA8"]. '</td>';
                    }
            }
            $sql6= "SELECT sum(mtA8) as somMt FROM designA8_sce_periode_data designA8_sce_periode_data where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and  codeDesignA8= $row1[code_designA8]";
...
function totalSce(){
    ...
    while ($row3 = mysqli_fetch_assoc($result3)){
        ...
        while(($row0 = mysqli_fetch_assoc($result0)) and $i<4) {
            $sql4 = "SELECT sum(mtA8) as somMt FROM designA8_sce_periode_data 
        ...
        //Total
        $sql8 = "SELECT sum(mtA8) as somMt FROM designA8_sce_periode_data 
...
//pdf 
function titleA8_pdf(){
    $output='</h3><br><br><br>                  
    <h3 style="text-align:center;text-transform: uppercase;">Entretien & Maintenance</h3>';
    return $output;
} 
//PDF-PAGE1
...
function designA8_pdf_p1(){
    ...
    $row10 = mysqli_fetch_assoc($result10);
            while($row1 = mysqli_fetch_assoc($result1)) {
               $row10 = mysqli_fetch_assoc($result10);
               $output .= '<tr style="font-size:7px;text-align:center;"> <td> ' . $row1["nomDesignA8"]. '</td>';
               $n='1';               
                while ($row3 = mysqli_fetch_assoc($result3)){
                    $sql0 = "SELECT code_sce,nomSce FROM service where codeSection= $row3[code_section] order by ordre";
                    $result0 = mysqli_query($conn, $sql0);
    ...
    $row10 = mysqli_fetch_assoc($result10);
            while($row1 = mysqli_fetch_assoc($result1)) {
               ...
               $n='1';               
                ...
                $sql6= "SELECT sum(mtA8) as somMt FROM designA8_sce_periode_data 
                where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']." and codeDesignA8= $row1[code_designA8]";
                $result6 = mysqli_query($conn, $sql6);
                $row6 = mysqli_fetch_assoc($result6);
                $output .= '<td>' .$row6["somMt"]. '</td>';
                $output .= '</tr>';
...
function totalSce_pdf_p2(){
    ...
    while ($row3 = mysqli_fetch_assoc($result3)){
        ...
        //Total
        $sql8 = "SELECT sum(mtA8) as somMt FROM designA8_sce_periode_data where periode='".$GLOBALS['periode']."' and trimestre='".$GLOBALS['trimestre']."' and annee=".$GLOBALS['annee']."";
        $result8 = mysqli_query($conn, $sql8);
        $row8 = mysqli_fetch_assoc($result8);
        $output .= '<td>'.$row8["somMt"]. '</td>';
        $output .= '</tr>';
...
if(isset($_POST["generate_pdf"]))  
 {  
      ...
      $content1.= designA8_pdf_p1();
      $content1.= totalSce_pdf_p1();
      $content1.='</table>';
      $obj_pdf->writeHTML($content1);  
      $obj_pdf->AddPage('L', 'A4');
      $content2 ='';
      $content2.= '<p></p><p></p><h3 style="text-align:right;text-transform: uppercase;">Tableau A8</h3><p></p>';
      $content2.= '<table border="1" cellspacing="0" cellpadding="3">';
      $content2.= sections_pdf_p2();
      $content2.= services_pdf_p2();
      $content2.= designA8_pdf_p2();
      $content2.= totalSce_pdf_p2();
      $content2.='</table>';      
      $obj_pdf->writeHTML($content2);  
      $obj_pdf->Output('A8_Trimestriel_'.$GLOBALS['trimestre'].'_'.$GLOBALS['annee'].'.pdf', 'I'); 
    }  
 ?>  
 <!DOCTYPE html>  
 <html>  
    ...
    <body>  
                ...
                ?>
                <br>
                    <h5 style="margin-left:78%;">Entretien & Maintenance</h5>
                    <h5 style="margin-left:78%;">TABLEAU A8 </h5>
                <table>
                <tr>
                ...
                <?php
                    ...
                    //Affichage des designA8- val_nbrerepas&Montant
                        $designA8= designA8();
                        echo "".$designA8."";
