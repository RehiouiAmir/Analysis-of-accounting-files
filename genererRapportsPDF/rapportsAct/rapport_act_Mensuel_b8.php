    ...
    $result1 = mysqli_query($conn, $sql1);    
            while($row1 = mysqli_fetch_assoc($result1)) {
               ...
               $output .= '<td> ' . $row_code["codeB8"]. '</td>';
                    ...
                    $result0 = mysqli_query($conn, $sql0);
                    while($row0 = mysqli_fetch_assoc($result0)) {
                        $sql2 = "SELECT nbreB8 FROM designB8_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB8=$row1[code_designB8]";
                        $result2 = mysqli_query($conn, $sql2);
                        $row2 = mysqli_fetch_assoc($result2);  
                        $output .= '<td>' . $row2["nbreB8"]. '</td>
                                    <td>' . $row2["nbreB8"]*$row_code['codeB8']. '</td>';
                    }
            $sql6= "SELECT sum(nbreB8) as somnbre FROM designB8_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeDesignB8= $row1[code_designB8]";
...
function totalSce(){
    ...
    $output .= '<tr  style="background-color: #BEBEBE;text-align:center;font-size:12px;"> <td  >TOTAL</td>';
        ...
        while(($row0 = mysqli_fetch_assoc($result0))) {
            ...
            $result1 = mysqli_query($conn, $sql1);
            while($row1 = mysqli_fetch_assoc($result1)) {
                $sql_sumNbr = "SELECT nbreB8,codeB8 FROM designB8_sce_periode_data where periode='".$GLOBALS['periode']."' and mois='".$GLOBALS['mois']."' and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB8=$row1[code_designB8]";
    ...
    $i = 0;
        ...
        while(($row0 = mysqli_fetch_assoc($result0))) {
            if ($GLOBALS['mois']=="Mars"){
                ...
                $result1 = mysqli_query($conn, $sql1);
                while($row1 = mysqli_fetch_assoc($result1)) {
                    $sql_sumNbr = "SELECT nbreB8,codeB8 FROM designB8_sce_periode_data where periode='".$GLOBALS['periode']."' 
                            and annee=".$GLOBALS['annee']." and  codeSce = $row0[code_sce] and codeDesignB8=$row1[code_designB8]
                            and ( mois='Mars' or mois='Fevrier' or mois='Janvier');";
                    $result_sumNbr = mysqli_query($conn, $sql_sumNbr);
                    $row_sumNbr = mysqli_fetch_assoc($result_sumNbr);
                    $sumNbr=$sumNbr+($row_sumNbr['nbreB8']*$row_sumNbr['codeB8']);
...
function titleB8_pdf(){
    ...
    $output='</h3><br><br><br>                  
            <h3 style="text-align:center;text-transform: uppercase;">ACTIVITE DE STOMATOLOGIE</h3>
...
if(isset($_POST["generate_pdf"]))  {  
     ...
     if ($GLOBALS['mois']=="Mars" or $GLOBALS['mois']=="Juin" or $GLOBALS['mois']=="Septembre" or $GLOBALS['mois']=="Decembre"){
      ...
      $obj_pdf->Output('B8_Mensuel_'.$GLOBALS['mois'].'_'.$GLOBALS['annee'].'.pdf', 'I'); 
    }  
 ?>  
 <!DOCTYPE html>  
 <html>  
